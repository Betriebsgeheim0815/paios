...
ALLOWED = set(REQUIRED) | {"sources", "reason"}
...
def validate_proposal(proposal):
  ...
  if not isinstance(provenance, dict) or not isinstance(provenance.get("actor"), str) or not provenance.get("actor").strip() or not isinstance(provenance.get("tool"), str) or not provenance.get("tool").strip() or not isinstance(provenance.get("recorded_at"), str):
